...
**Re-render granularity.** In both modes the user-visible guarantee is the same: a data-bound value change (for example, a `Text` node's bound `text` property, or a `TextField`'s two-way-bound `value`) produces correct final content with no flicker and no user-visible artifact, and any part of the tree unrelated to the change is left completely untouched — its DOM elements, focus, scroll position, and any CSS-transition state all survive. In adapter mode this now holds at the level of the *changed* element too, for both passive and interactive nodes: a **bound-value-only change** — the wire-level property definition is unchanged and only the value behind an existing `{path: ...}` binding changed, via `updateDataModel` — patches the existing element in place through `context.subscribe()` (`dataContext.subscribeDynamicValue`), the same mechanism `_bindValue` already uses in every `_render*` method. The element's own DOM node, and therefore its focus and cursor position for an `<input>`/`<textarea>`, is preserved across the update; typing into a two-way-bound `TextField` (or `CheckBox`, `Switch`, `Slider`, `DateTimeInput`, `ChoicePicker`, `Autocomplete`, and any other Checkable control) does not lose focus on any keystroke.

A **genuinely structural change**, by contrast, still rebuilds the node fresh and swaps it in with `replaceWith()` via its `watchNode` effect: the root replaced; list items added, removed, or reordered; a child upgrading from a placeholder once its definition arrives; or a component resent with a changed *static* (non-data-bound) literal property, since a literal has no `subscribe()` path to patch it through. `watchNode` tells these apart by comparing, on every `node.props` change, (a) the identity of the node's own raw wire-level properties (`surface.componentsModel.get(id).properties`, which only changes on a fresh `updateComponents` for that id) and (b) a snapshot of its resolved ref-field (child-pointer) values — a rebuild fires when either differs from the last run, and is skipped (leaving the change to `subscribe()`) when neither does. Structural rebuilds are keyed by the node's own identity in the resolver's tree, exactly as before.
